...
const CumulativeGPACalculator: React.FC<CumulativeGPACalculatorProps> = ({ navigateTo }) => {
  ...
  // Print function (from Unweighted GPA Calculator - working version)
  const handlePrint = () => {
          ...
          <table>
            ...
            <tbody>
              ${validSemesters.map(semester => {
                ...
                const sanitizedName = (semester.name || 'Unnamed Semester').replace(/</g, "&lt;").replace(/>/g, "&gt;");
  ...
  // Download function (from Unweighted GPA Calculator - working version)
  const handleDownload = () => {
    const validSemesters = semesters.filter(s => s.semesterGPA > 0 && s.creditsEarned > 0);

    let textContent = `Cumulative GPA Calculator Report\n`;
    textContent += `Generated on: ${new Date().toLocaleDateString()}\n\n`;

    textContent += `=== Semesters ===\n`;
    validSemesters.forEach(semester => {
      const qualityPoints = semester.semesterGPA * semester.creditsEarned;
      textContent += `- ${semester.name || 'Unnamed Semester'}: ${semester.semesterGPA.toFixed(2)} GPA (${semester.creditsEarned} credits, ${qualityPoints.toFixed(2)} quality points)\n`;
    });
    textContent += `\n`;

    textContent += `=== Results ===\n`;
    if (cumulativeGPA !== null) {
      textContent += `Cumulative GPA: ${cumulativeGPA.toFixed(2)} / 4.0\n`;
      textContent += `Total Credits Earned: ${totalCredits}\n`;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 dark:from-gray-900 dark:via-blue-900 dark:to-cyan-900">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-blue-600 via-cyan-600 to-green-600 bg-clip-text text-transparent mb-4">
