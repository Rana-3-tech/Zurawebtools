...
const ASUGPACalculator: React.FC<ASUGPACalculatorProps> = ({ navigateTo }) => {
  ...
  const handlePrint = () => {
    ...
    const html = `
      ...
      <html>
        <head>
          <title>ASU GPA Calculator Report</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 20px; }
            .header { text-align: center; margin-bottom: 30px; }
            .header h1 { color: #8C1D40; }
            .current-status { background-color: #f9f4e8; padding: 20px; border-radius: 8px; margin-bottom: 30px; border-left: 4px solid #8C1D40; }
            .gpa-display { font-size: 24px; font-weight: bold; color: #8C1D40; text-align: center; margin: 20px 0; }
            .section { margin-bottom: 30px; border: 1px solid #ddd; padding: 20px; border-radius: 8px; }
            table { width: 100%; border-collapse: collapse; margin-top: 10px; }
            th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }
            th { background-color: #8C1D40; color: white; }
            .projected { background-color: #e8f4f8; padding: 20px; border-radius: 8px; margin-top: 20px; }
            .footer { text-align: center; margin-top: 30px; color: #666; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h1>ASU GPA Calculator Report</h1>
          ...
          <div class="current-status">
            ...
            <p><strong>Current GPA:</strong> ${currentGPA || 'N/A'}</p>
            <p><strong>Current Units:</strong> ${currentUnits || 'N/A'}</p>
            <p><strong>Current Grade Points:</strong> ${currentGradePoints || 'N/A'}</p>
          </div>
          ...
          <div class="section">
            ...
            ` : '<p>No active courses entered.</p>'}
          </div>

          <div class="projected">
            <h2>Projected GPAs</h2>
            ${projectedSemesterGPA !== null ? `
              <div class="gpa-display">
                Projected Semester GPA: ${projectedSemesterGPA.toFixed(2)}
              </div>
            ` : ''}
            ${projectedCumulativeGPA !== null ? `
              <div class="gpa-display">
                Projected Cumulative GPA: ${projectedCumulativeGPA.toFixed(2)}
              </div>
            ` : ''}
          </div>

          <div class="footer">
            <p>Generated by ZuraWebTools ASU GPA Calculator</p>
            <p>Note: This calculator uses the ASU 4.0 grading scale (A+ to E)</p>
          </div>
        </body>
      </html>
    `;
  ...
  const handleDownload = () => {
    ...
    let textContent = `ASU GPA Calculator Report\n`;
    textContent += `Arizona State University\n`;
    textContent += `Generated on: ${new Date().toLocaleDateString()}\n\n`;

    textContent += `=== Current Academic Standing ===\n`;
    textContent += `Current GPA: ${currentGPA || 'N/A'}\n`;
    textContent += `Current Units: ${currentUnits || 'N/A'}\n`;
    textContent += `Current Grade Points: ${currentGradePoints || 'N/A'}\n\n`;

    textContent += `=== Current Semester Courses ===\n`;
    if (activeCourses.length > 0) {
      activeCourses.forEach(course => {
        textContent += `- ${course.name || 'Unnamed Course'}: ${course.grade} (${course.units} units, ${course.gradePoints.toFixed(2)} grade points)\n`;
      });
    } else {
      textContent += `No active courses entered.\n`;
    }
    textContent += `\n`;

    textContent += `=== Projected GPAs ===\n`;
    if (projectedSemesterGPA !== null) {
      textContent += `Projected Semester GPA: ${projectedSemesterGPA.toFixed(2)}\n`;
    }
    if (projectedCumulativeGPA !== null) {
      textContent += `Projected Cumulative GPA: ${projectedCumulativeGPA.toFixed(2)}\n`;
    }

    textContent += `\n---\n`;
    textContent += `Generated by ZuraWebTools ASU GPA Calculator\n`;
    textContent += `Note: This calculator uses the ASU 4.0 grading scale (A+ to E)\n`;

    const blob = new Blob([textContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `ASU_GPA_Report_${new Date().toISOString().split('T')[0]}.txt`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 py-12 px-4">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <button
                ...
                aria-label="Print ASU GPA report"
              >
              ...
              <button
                ...
                aria-label="Download ASU GPA report as text file"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 rounded-xl shadow-lg p-6 mb-8 border border-purple-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {Object.entries(gradeScale).map(([grade, points]) => (
              <div key={grade} className="bg-gradient-to-br from-white to-purple-50 rounded-lg p-3 border border-purple-200 text-center hover:border-[#8C1D40] hover:shadow-md transition-all">
                <div className="text-2xl font-bold text-gray-900">{grade}</div>
                <div className="text-sm text-purple-600 font-medium">{points.toFixed(2)}</div>
              </div>
            ))}
          </div>
          <p className="text-sm text-gray-800 mt-4 bg-white/50 rounded-lg p-3 border-l-4 border-purple-400">
        ...
        <section id="benefits" className="bg-white rounded-xl shadow-lg p-6 mb-8 border border-slate-200 scroll-mt-24">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-gradient-to-br from-cyan-50 to-cyan-100 p-6 rounded-lg border border-cyan-200">
              ...
              <p className="text-slate-600">Print reports or download as text files for your academic records and planning.</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-pink-50 to-pink-100 p-6 rounded-lg border border-pink-200">
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2">Goal Planning</h3>
              <p className="text-slate-600">Test different grade scenarios to understand what you need to achieve your target GPA.</p>
            </div>
          </div>
        </section>
        ...
        <section id="how-to-use" className="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-200 scroll-mt-24">
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              <div className="flex-shrink-0">
                <div className="w-10 h-10 bg-gradient-to-br from-[#8C1D40] to-[#FFC627] rounded-full flex items-center justify-center text-white font-bold text-lg">5</div>
              </div>
              <div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">Save or Print Your Report</h3>
                <p className="text-gray-700 leading-relaxed mb-3">
                  Use the export buttons below the calculator to save your results:
                </p>
                <ul className="list-disc ml-6 space-y-2 text-gray-700">
                  <li><strong>Print Report:</strong> Opens a print-friendly version with all your course details and GPA calculations</li>
                  <li><strong>Download Report:</strong> Saves your results as a text file for your records or academic planning</li>
                </ul>
                <div className="mt-3 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg">
                  <p className="text-sm text-gray-700"><strong>💾 Pro Tip:</strong> Download your report before making changes to compare different course scenarios and grade outcomes.</p>
                </div>
              </div>
            </div>
          </div>
